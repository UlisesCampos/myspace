...
func CommentCreate(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		m.Message = fmt.Sprintf("Error al leer el comentaio: %s", err)
	...
	if err != nil {
		m.Code = http.StatusBadRequest
		m.Message = fmt.Sprintf("Error al registrar el comentaio: %s", err)
		commons.DisplayMessage(w, m)
		return
	}
	...
	origin := fmt.Sprintf("http://lovalhost:%d/", commons.Port)
...
// ComentGetAll contiene todos los comentarios
